...
from app.modules.jobs.repository import VALID_JOB_STATUSES, _job_filters
...
def test_job_filters_include_status_when_requested():
    ...
    assert "job_postings.status" in where
    assert compiled.params["status_1"] == "applied"
...
def test_job_filters_omit_status_for_all_jobs():
    ...
    assert "job_postings.tenant_id" in where
    assert "job_postings.status" not in where


def test_job_status_set_matches_public_api_contract():
    assert VALID_JOB_STATUSES == {"new", "saved", "applied"}
