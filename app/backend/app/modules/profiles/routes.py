...
async def import_profile_from_url(
    ...
    profile = await upsert_profile(db, tenant.id, name, headline, cv_md, location=location)
...
async def import_profile_from_pdf(
    ...
    profile = await upsert_profile(db, tenant.id, name or "Unknown", headline or "Unknown", cv_md)
...
async def upload_cv(
    ...
    existing = await get_profile_by_tenant(db, tenant.id)
    ...
    profile = await upsert_profile(db, tenant.id, name, headline, body.raw_cv_md, location=location)
