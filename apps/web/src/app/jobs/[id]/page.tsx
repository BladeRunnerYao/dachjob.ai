...
export default function JobDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.getJob(id),
      api.getMatchReport(id),
      api.getResumeArtifact(id),
    ]).then(([j, m, r]) => {
      setJob(j);
      setMatch(m);
      setResume(r);
          ...
          {job.parsed_json ? (
            <>
              <Card>
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    {(job.parsed_json.skills as string[] || []).map((s: string) => (
              ...
              <Card>
                ...
                <CardContent>
                  <ul className="list-disc list-inside space-y-1 text-sm text-slate-700">
                    {(job.parsed_json.responsibilities as string[] || []).map((r: string, i: number) => (
                      <li key={i}>{r}</li>
                    ))}
                  </ul>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <p className="text-sm text-slate-700">{job.parsed_json.years_exp as string}+ years</p>
                </CardContent>
              </Card>
            </>
          ) : (
